...
import Cheapflight from "/src/components/universal/cheapflight";
import FlightCard from "/src/components/universal/flightcard";
import Newsletter from "/src/components/universal/newsletter";
import React, { useEffect, useRef, useState, Suspense } from "react";
import styled from "styled-components";
import Flightnavbar from "/src/components/navigation/flightnavbar";
import Flightitenary from "/src/components/modals/flightitenary";
import HttpServices from "/src/services/HttpServices";
import ApiRoutes from "/src/services/ApiRoutes";
import FlightLoader from "/src/components/universal/flightLoader";
...
import FlightFilterComponent from "/src/components/universal/FlightFilterComponent"
import FlightOffer from "./flightOffer";
import { BackToTop } from "/src/components/utils/functions";
import { AlertError } from "/src/components/utils/functions";
import { dispatchRefundable } from "/src/app/dataSlice";
import { FlightRequest } from "/src/components/universal/flightcard";
import moment from "moment";
import { FilterMultiCityRefundables } from "/src/components/utils/functions";
import { TripName } from "/src/components/universal/flightcard";
import { ClientGetApi, MainApi } from "/src/services/MainApi";
import { FirebaseImage } from "/src/components/utils/functions";
import { BannerTypes } from "/src/components/utils/functions";
...
function FlightSelector({ onDeals }) {
  ...
  const handleAds = async (id) => {
    try {
      await ClientGetApi(`${MainApi.landing.impression}/${id}/`)
      const response = await ClientGetApi(`${MainApi.landing.click}/${id}/`)
      if (response.status === 200) {
        const preUrl = !response.data.url.includes('https://') ? 'https://' : ''
        return window.open(`${preUrl}${response.data.url}`, "_blank")
      }
    } catch (e) {
      AlertError(`${e}`)
    }
  ...
  const AdsView = () => {
    return (
      <>
        <BottomCard
          onClick={() => handleAds(ads.id)}
          style={{ backgroundImage: `url(${FirebaseImage(ads.media)})` }}
          className="cursor-pointer"
        >
          <ExploreWrapper>
            <Small dangerouslySetInnerHTML={{ __html: ads?.title || 'Book a Flight from around the world' }} />
          </ExploreWrapper>
        </BottomCard>
      </>
    )
  ...
  useEffect(() => {
    const FetchBanner = async () => {
      try {
        const res = await ClientGetApi(MainApi.landing.all)
        const adsArr = res.data.find(ele => ele.banner_type === BannerTypes[2] && ele.published === true)
        setAds(adsArr)
      } catch (error) {
        AlertError(`${error}`)
      } finally {
            ...
            <SectionHolder>
              <Section>
                <LeftSection>
                  ...
                  <div className="">
                    <div className="w-11/12 mx-auto max-w-[1165px] mt-10">
                      {AdsView()}
                    </div>
                  </div>
                </LeftSection>
