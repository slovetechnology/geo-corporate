...
export default function Forminput({ type = "text", placeholder, name, error, setup, dailcode }: Props) {
    ...
    if (singleforms.includes(type)) return (
        <div className='relative mb-5'>
            <Field type={type} name={name} placeholder={placeholder} className={`border w-full transition-all p-3.5 rounded-md outline-none ${error ? 'border-red-600' : 'border-[#383838B3]'}`} />
    ...
    return (
        <div className="grid grid-cols-7 gap-3 relative">
            ...
            <div className="col-span-5">
                <div className='relative mb-5'>
                    <Field type={'number'} name={name} placeholder={placeholder} className={`border w-full transition-all p-3.5 rounded-md outline-none ${error ? 'border-red-600' : 'border-[#383838B3]'}`} />
                    {error && <div className={`text-red-500 text-sm absolute left-0`}>{error}</div>}
                </div>
            </div>
        </div>
