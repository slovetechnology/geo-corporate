...
export default function LoginOtpRequest() {
    ...
    const SendVerificationCode = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.status === 200){
                setScreen(1)
                return setMsg({ status: 'success', message: `${response.message}` })
            }else {
                return setMsg({ status: 'error', message: `${response.message}` })
            }
        } catch (error: any) {
            return setMsg({ status: 'error', message: `${error.message}` })
        } finally {
            ...
            {screen === 0 && <div className="w-full">
                <form onSubmit={SendVerificationCode} className="">
                    ...
                    <div className="text-center w-3/5 mx-auto font-light mb-10">Let's help you get verified</div>
                        ...
                        <Formbutton type="submit" title="Request for verification" loading={loading} />
                </form>
            </div>}
